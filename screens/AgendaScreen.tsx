...
export const AgendaScreen: React.FC = () => {
  return (
    <div className="flex flex-col h-screen bg-background-light dark:bg-background-dark overflow-hidden">
      {/* Header */}
      <header className="px-6 py-4 flex items-center justify-between shrink-0 bg-background-light dark:bg-background-dark z-10">
        <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Agenda</h1>
        <button className="p-2 rounded-full bg-surface-light dark:bg-surface-dark border border-gray-200 dark:border-gray-700 shadow-sm relative">
          ...
          <div className="grid grid-cols-7 text-center gap-y-2">
            ...
            {[1, 2, 3, 4, 5].map(d => (
              <button key={d} className="h-9 w-9 mx-auto flex flex-col items-center justify-center rounded-lg hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors relative text-gray-700 dark:text-gray-300 text-sm">
                {d}
                {d === 3 && <span className="w-1 h-1 bg-primary rounded-full absolute bottom-1"></span>}
              </button>
            ))}
            <button className="h-9 w-9 mx-auto flex flex-col items-center justify-center rounded-lg bg-primary text-white shadow-md shadow-primary/30 relative text-sm font-bold">
              6
      ...
      <div className="px-6 pb-2 shrink-0 overflow-x-auto no-scrollbar">
        <div className="flex gap-3">
          <button className="px-4 py-1.5 rounded-full bg-primary text-white text-sm font-medium shadow-sm whitespace-nowrap">Alle events</button>
          <button className="px-4 py-1.5 rounded-full bg-white dark:bg-surface-dark border border-gray-100 dark:border-gray-800 text-gray-600 dark:text-gray-300 text-sm font-medium whitespace-nowrap">Mijn taken</button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-24 pt-2 space-y-4 no-scrollbar">
        ...
        {/* Card 1 */}
        <div className="bg-white dark:bg-surface-dark rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col gap-3 relative overflow-hidden group">
          <div className="absolute top-0 left-0 w-1 h-full bg-primary"></div>
          <div className="flex justify-between items-start">
            <div>
              <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-primary/10 text-primary mb-1 uppercase tracking-wide">KSA Nationaal</span>
              <h4 className="text-lg font-bold text-gray-900 dark:text-white leading-tight">Leidingsweekend</h4>
            </div>
            <div className="text-right">
              <span className="block text-xl font-semibold text-primary dark:text-blue-400">20:00</span>
        ...
        {/* Card 2 */}
        <div className="bg-white dark:bg-surface-dark rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col gap-3 relative overflow-hidden">
      ...
      <div className="absolute bottom-24 right-6 z-20">
        <button className="group w-14 h-14 bg-primary rounded-full shadow-lg shadow-primary/40 flex items-center justify-center transition-all hover:scale-105 active:scale-95">
