...
interface NudgeSelectorScreenProps {
  onBack: () => void;
}

export const NudgeSelectorScreen: React.FC<NudgeSelectorScreenProps> = ({ onBack }) => {
  const [search, setSearch] = useState('');
  const [nudgedIds, setNudgedIds] = useState<string[]>(['3']); // Thomas pre-nudged as per screenshot
  const [showToast, setShowToast] = useState(false);

  const leaders = [
    { id: '1', name: 'Jan De Smet', role: 'Hoofdleiding', status: 'online', avatar: 'https://i.pravatar.cc/150?u=jan' },
    { id: '2', name: 'Sarah Vermeulen', role: 'Penningmeester', status: 'offline', avatar: 'https://i.pravatar.cc/150?u=sarah' },
    { id: '3', name: 'Thomas De Vries', role: 'Leiding', status: 'offline', avatar: 'https://i.pravatar.cc/150?u=thomas' },
    { id: '4', name: 'Lars Peeters', role: 'Materiaalmeester', status: 'online', avatar: 'https://i.pravatar.cc/150?u=lars' },
    { id: '5', name: 'Kevin Janssen', role: 'Leiding', status: 'offline', avatar: 'https://i.pravatar.cc/150?u=kevin' },
    { id: '6', name: 'Emma Verhaegen', role: 'Leiding', status: 'offline', avatar: 'https://i.pravatar.cc/150?u=emma' },
  ];
    ...
    <div className="flex flex-col min-h-screen bg-[#0f172a] text-white font-sans relative">
      ...
      <header className="px-4 py-4 sticky top-0 bg-[#0f172a] z-10">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">Herinner leiding aan hun streepjes</p>
          </div>
        </div>
      ...
      <main className="flex-1 px-4 pb-24 overflow-y-auto">
        <div className="flex justify-between items-center mb-4 mt-2">
          <h2 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Alle Leiding</h2>
          <span className="text-[10px] bg-blue-900/30 text-blue-400 px-2 py-0.5 rounded border border-blue-900/50">{leaders.length} Actief</span>
        </div>
            ...
            return (
              <div key={leader.id} className="flex items-center justify-between p-2">
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <img src={leader.avatar} alt={leader.name} className="w-12 h-12 rounded-full object-cover border-2 border-[#1e293b]" />
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{leader.role}</p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2 transition-all ${
                    isNudged 
                      ? 'bg-[#1e293b] text-gray-500 cursor-default' 
      ...
      {showToast && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-4 fade-in duration-300">
          <div className="bg-white text-gray-900 px-6 py-4 rounded-xl shadow-2xl flex items-center gap-4 min-w-[300px]">
