    ...
    <div className="flex flex-col min-h-screen bg-[#0f172a] text-white font-sans">
      ...
      <main className="flex-1 px-4 pb-24 overflow-y-auto">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="bg-[#1e3a8a]/40 border border-blue-900 rounded-xl p-3 flex flex-col justify-between h-24">
             <span className="text-[10px] text-blue-200 font-bold uppercase tracking-wider">Totaal Vaten</span>
             <div className="flex justify-between items-end">
                <span className="text-2xl font-bold text-white">12</span>
                <span className="material-icons-round text-blue-500 text-lg opacity-50">propane_tank</span>
             </div>
          </div>
          <div className="bg-[#2a1c15] border border-orange-900/50 rounded-xl p-3 flex flex-col justify-between h-24">
             <span className="text-[10px] text-orange-200 font-bold uppercase tracking-wider">Vervalt Snel</span>
             <div className="flex justify-between items-end">
                <span className="text-2xl font-bold text-orange-500">3</span>
                <span className="material-icons-round text-orange-500 text-lg opacity-50">warning</span>
             </div>
          </div>
          <div className="bg-[#1e293b] border border-gray-700 rounded-xl p-3 flex flex-col justify-between h-24">
             <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Snacks</span>
             <div className="flex justify-between items-end">
        ...
        <div className="flex justify-between items-end mb-3">
          ...
          <button className="text-blue-400 text-xs font-bold hover:underline">Alles tonen</button>
        </div>

        {/* List Items */}
        <div className="space-y-3">
          {stockItems.map(item => (
            <div key={item.id} className="bg-[#1e293b] p-3 rounded-xl border border-gray-800 flex gap-3 group">
              {/* Icon Box */}
              <div className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 ${item.id === 3 ? 'bg-yellow-100' : 'bg-gradient-to-br from-gray-700 to-gray-800'} overflow-hidden relative shadow-inner`}>
                <span className={`material-icons-round text-2xl ${item.id === 3 ? 'text-yellow-600' : 'text-white/80'}`}>{item.icon}</span>
              </div>
              
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start">
                  <h3 className="font-bold text-white text-sm truncate pr-2">{item.name}</h3>
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${item.urgent ? 'bg-orange-500/20 text-orange-500' : 'bg-gray-700 text-gray-400'}`}>
                    Exp: {item.exp}
                  </span>
                </div>
                
                <div className="flex items-center gap-2 mt-1 mb-1">
                   <span className="text-[10px] bg-blue-900/30 text-blue-300 px-1.5 py-0.5 rounded border border-blue-900/50">{item.label}</span>
                   {item.location && <span className="text-[10px] text-gray-400">Locatie: {item.location}</span>}
                </div>
              </div>

              <div className="text-right flex flex-col justify-center min-w-[3rem]">
                 <span className="text-lg font-bold text-white leading-none">{item.count}</span>
      ...
      <footer className="fixed bottom-0 left-0 right-0 p-4 bg-[#0f172a] border-t border-gray-800 z-20 flex gap-3">
         ...
         <button className="w-14 bg-[#1e293b] rounded-xl flex items-center justify-center border border-gray-700 text-gray-300">
