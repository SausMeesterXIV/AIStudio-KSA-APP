...
export const QuotesScreen: React.FC<QuotesScreenProps> = ({ 
  ...
  const filteredAuthors = MOCK_USERS.filter(u => 
    (u.nickname || u.name).toLowerCase().includes(authorSearch.toLowerCase())
  );
  ...
  const handleSelectAuthor = (u: typeof MOCK_USERS[0]) => {
    setSelectedAuthor({id: u.id, name: u.nickname || u.name});
    setAuthorSearch('');
                    ...
                    {viewMode === 'current' ? (
                        <div className="flex items-center bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
                            ...
                            <button 
                                ...
                                className={`flex items-center gap-1 px-2 py-1.5 rounded-md transition-all active:scale-95 ${
                                    hasLiked 
